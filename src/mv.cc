...
******************************************************************************
 * @file mv.cc
 * @brief MV implementation source file
 * @author Janani Kannan
 * @author Leo Dong
 * @author Alex Johnson
 * @author Ashwin Wariar
 * @bug rewrite me, I am just opl right now
******************************************************************************
*/
...
void MV::processElectables() {
  ...
  while (k < this->candidateCount) {
    // ***************************************************
    // find [candidate,party] pair
    end_idx = line.find("]");
    std::string pair = line.substr(1, end_idx);
    ...
    // check that ballot doesn't exceed number of seats
...
std::vector<Party*> MV::getParties() {
  return this->parties;
}

std::vector<Candidate*> MV::getWinners() {
